void main() {
  ...
  bool undo = true;    //has the dislike button been unpressed?
  ...
  if (dislike && undo) {
    userMoodList = undoQueryRanges(eVal, dVal, aVal, mood, userID);
    print(userMoodList);
  } 
}
...
/*
   ...
   * <p>
   * Method takes the given mood and determines which mood query range
   ...
   * @param userID user identifier for their specific mood query ranges
   * 
   * @return       list of [new_joyRanges,new_sadRanges,new_madRanges]
 */

List undoQueryRanges(
  ...
  switch (mood) {
    case "joy":
      {
        joyList = undoJoyParams(eVal, dVal);
      }
      break;

    case "sadness":
      {
        sadList = undoSadParams(eVal, dVal);
      }
      break;

    case "anger":
      {
        angerList = undoAngerParams(eVal, aVal);
      }
      break;
...
/*
   * updates user query ranges for Joy
   * <p>
...
List undoJoyParams(num eVal, num dVal) {
  ...
  */
  ...
  List eRange = undoEnergyRange(eVal, eValMax, eValMin, valInc);
  List dRange = undoDanceRange(dVal, dValMax, dValMin, valInc);

  return (eRange + dRange);
}

/*
   * updates user query ranges for Sadness
   * <p>
...
List undoSadParams(num eVal, num dVal) {
...
/*
   * updates user query ranges for Anger
   * <p>
...
List undoAngerParams(num eVal, num aVal) {
  // To-do: fetch user querry ranges from database here
  ...
  */
  ...
  List eRange = undoEnergyRange(eVal, eValMax, eValMin, valInc);
  List aRange = undoAcousticRange(aVal, aValMax, aValMin, valInc);

  return (eRange + aRange);
}

/*
   * updates user query ranges for Energy
   * <p>
   * Method takes the given song's energy value from Spotify and 
   * determines where in the current set query range the given value
...
List undoEnergyRange(num eVal, num eValMax, num eValMin, num valInc) {
...
/*
   * updates user query ranges for Dancability
   * <p>
...
List undoDanceRange(num dVal, num dValMax, num dValMin, num valInc) {
...
/*
   * updates user query ranges for Acousticness
   * <p>
...
List undoAcousticRange(num aVal, num aValMax, num aValMin, num valInc) {
